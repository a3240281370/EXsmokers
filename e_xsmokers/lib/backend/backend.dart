...
import 'schema/background_icon_media_record.dart';
...
export 'schema/background_icon_media_record.dart';
...
/// Functions to query BackgroundIconMediaRecords (as a Stream and as a Future).
Future<int> queryBackgroundIconMediaRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      BackgroundIconMediaRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<BackgroundIconMediaRecord>> queryBackgroundIconMediaRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      BackgroundIconMediaRecord.collection,
      BackgroundIconMediaRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<BackgroundIconMediaRecord>> queryBackgroundIconMediaRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<BackgroundIconMediaRecord>>
    queryBackgroundIconMediaRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, BackgroundIconMediaRecord>
      controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
        queryCollectionPage(
          ...
          if (isStream) {
            final streamSubscription = (page.dataStream)
                ?.listen((List<BackgroundIconMediaRecord> data) {
              data.forEach((item) {
                final itemIndexes = controller.itemList!
                    .asMap()
                    .map((k, v) => MapEntry(v.reference.id, k));
...
Future<FFFirestorePage<HobbiesRecord>> queryHobbiesRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, HobbiesRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      HobbiesRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<HobbiesRecord> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
